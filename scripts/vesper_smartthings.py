#!/usr/bin/env python3
"""
VESPER ObjectNav Demo with SmartThings + Docker Firmware Integration.

First-person navigation with realistic walking humanoid avatar (Habitat 3.0 style).
Uses KinematicHumanoid and HumanoidRearrangeController for proper walk animations.

SmartThings bi-directional synchronization:
- Each room has a Docker container running real ARM Cortex-M3 firmware in QEMU
- SmartThings Schema Connector exposes devices to the SmartThings phone app
- When the humanoid gets close to a device in 3D, it triggers interaction
- Phone app commands (on/off) reflect instantly in the 3D environment
"""
...
class SmartThings3DBridge:
    ...
    def start(self, room_positions: Dict[str, Tuple[float, float, float]]):
        ...
        if not self._port_available(self.port):
            logger.warning(f"[ST-3D] Port {self.port} in use — trying to free it")
    ...
    async def _async_start(self, room_positions: Dict[str, Tuple[float, float, float]]):
        ...
        for idx, room_name in enumerate(rooms):
            dev_id = f"vesper-3d-{room_name.replace(' ', '-').replace('.', '-')}"
...
def main():
    ...
    # Try to load humanoid for third-person view FIRST (before VESPER which may timeout)
    print("\n=== Loading humanoid for third-person view ===", flush=True)
    demo.load_humanoid()
    print(f"=== Humanoid loaded: {demo.humanoid is not None} ===", flush=True)
    
    # Initialize VESPER integration with IoT devices
    print("\nInitializing VESPER integration...")
    demo.init_vesper_integration(list(room_positions.keys()), room_pos_dict)
    
    # Initialize SmartThings ↔ 3D Bridge (Docker containers + cloud sync)
    if room_pos_dict:
        print("\nInitializing SmartThings bridge...")
    ...
    print("Controls: WASD move | T task | N auto-nav | P SmartThings | F toggle light | V view | ESC quit")
    ...
    while running:
        ...
        if action == "quit":
            ...
            # Toggle the nearest SmartThings device
            if demo.smartthings_bridge:
                agent_state = demo.agent.get_state()
                agent_pos = tuple(agent_state.position)
                ...
                if closest:
                    ...
                    if fw:
                        ...
                        if evt:
                            new_st = evt["new_state"]
                            ui.status_message = f"💡 {fw.room.title()} Light → {new_st.upper()} ({dist:.1f}m)"
                        else:
                            ui.status_message = "Could not toggle device"
                    else:
                        ui.status_message = "No firmware device found"
                else:
                    ui.status_message = "No SmartThings devices nearby"
            else:
                ui.status_message = "SmartThings bridge not active"
            ...
            # Determine if moving and calculate target for walking animation
            ...
            if is_moving and ui.goal_pos is not None:
                ...
                # Moving without a goal - just walk forward
                agent_state = demo.agent.get_state()
                forward_target = mn.Vector3(agent_state.position[0], 0, agent_state.position[2] - 1)
                demo.update_humanoid_position(is_moving=True, target_pos=forward_target)
            else:
                # Turning in place
        ...
        if ui.auto_navigate and ui.goal_pos is not None and action is None:
            ...
            if auto_action:
                ...
                # Walking animation while navigating
                is_moving = auto_action in ["move_forward", "move_backward"]
                demo.update_humanoid_position(is_moving=is_moving, target_pos=mn.Vector3(*ui.goal_pos))
    ...
    # Stop SmartThings bridge (Docker containers + webhook)
    if demo.smartthings_bridge:
    ...
    print("\nDemo ended.")
    logger.info("=" * 80)
    logger.info("VESPER ObjectNav Demo - Session Ended Successfully")
    logger.info(f"Log file saved: {log_filepath}")
    logger.info("=" * 80)
...
if __name__ == "__main__":
    try:
        ...
        logger.error("=" * 80)
        logger.error("FATAL ERROR - Demo crashed!")
